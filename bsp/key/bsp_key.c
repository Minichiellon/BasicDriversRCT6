...
void CheckKeyEvent(KeyIndex key)
{
    uint8_t buttonValue;
	if(key == KEY_1)
		buttonValue = GPIO_ReadInputDataBit(KEY_1_GPIO, KEY_1_PIN); // 读取按键状态（按下为LOW）
	else if(key == KEY_2)
		buttonValue = GPIO_ReadInputDataBit(KEY_2_GPIO, KEY_2_PIN); // 读取按键状态（按下为LOW）
	else if(key == KEY_3)
		buttonValue = GPIO_ReadInputDataBit(KEY_3_GPIO, KEY_3_PIN); // 读取按键状态（按下为LOW）
